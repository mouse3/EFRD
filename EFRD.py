...
class EFRD_Protocol_v4_1:
    def __init__(self, PIB_Y, Gini, Alpha, Sigma, Limite_L, G_op, IPC_Pi,
        ...
        Parámetro `modo`:
          ...
          'test'        → no ejecuta ningún protocolo de emergencia (solo calcula).
        """
    ...
    def _calcular_x(diferencial: float, k_hogar: float) -> float:
        """
        Exceso relativo de renta según la definición canónica del README.
        Si k_hogar es 0 o negativo, el hogar carece de umbral definido:
        toda la renta es excedente → x se satura hacia +∞.
        """
        if k_hogar > 0:
            return diferencial / k_hogar
        # Caso borde documentado: sin umbral, máxima contribución posible.
        # x=10 → tasa ≈ L*(1 - e^-15) ≈ L (saturado)
        return 10.0

    def _cargar_datos_desde_db(self):
        """
        Recorre la DB explotada agrupando dinámicamente por vivienda en SQL.
    ...
    def _ejecutar_logica_central(self):
        """
        Fleco 8: lógica de emergencia desacoplada del input().
        El comportamiento depende de self.modo.
        """
        ...
        if saldo_actual < 0:
            ...
            if self.modo == "interactivo":
                ...
                opcion = "1"  # Deuda como valor seguro por defecto
        ...
        if self.k_base < self.k_arope:
            ...
            if ajuste_emergencia_activado:
                ...
                if saldo_psd >= 0:
                    ...
                    # En modos no interactivos, no se activa PSD automáticamente (política conservadora)
                else:
    ...
    def _pedir_decision_insolvencia(self, k_equilibrio: float) -> str:
        """Fleco 8: aísla el input() de insolvencia, fácil de mockear en tests."""
    ...
    def _pedir_decision_psd(self):
        """Fleco 8: aísla el input() del Protocolo PSD."""
    ...
    def _verificar_masa_critica(self):
        """
        Fleco 15: Protocolo de Masa Crítica.
        Verifica que el ratio de contribuyentes activos supera el umbral mínimo de sostenibilidad.
        """
...
class EFRD_AnalyticVisualizer:
    ...
    def graficar_curva_sostenibilidad(self, rango_k=None,
        ...
        plt.axvline(self.motor.k_base, color="purple", linestyle="-.",
                    label=f"k_base Actual ({self.motor.k_base:.2f}€)")
    ...
    def graficar_distribucion_ingresos(self, bins=50,
        ...
        """
        Gráfica de distribución real de ingresos brutos anuales desde la BD.
        Fleco 3 y Fleco 6: tabla dinámica, acceso SQLite como camino único y limpio.
        """
        import numpy as np
        import sqlite3
